 ...
 */
package org.eclipse.daanse.sql.dialect.db.duckdb.sqlgen;
...
/**
 * DuckDB tracks the SQL-99 default for table/column/view rename; index and
 * constraint rename have no DuckDB equivalent, and sequence rename is left
 * off despite DuckDB having sequences (verified unsupported).
 */
class DuckDbAlterRenameOfflineTest {
    ...
    private final DuckDbDialect dialect = new DuckDbDialect();
    ...
    @Test
    void renameView_inherits_ANSI_default() {
        assertThat(dialect.ddlGenerator().supportsRenameView()).isTrue();
